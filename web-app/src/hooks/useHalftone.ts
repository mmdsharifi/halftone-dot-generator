...
export const useHalftone = (
  ...
  useEffect(() => {
    if (!dotsRef.current.length || !canvasSize.width || !canvasSize.height)
      return;
    const svg = generateSvgString(
      dotsRef.current,
      canvasSize.width,
    ...
    setSvgString(svg);
  }, [animationSettings, canvasSize.height, canvasSize.width, settings]);
  ...
  }, [animationSettings, settings, canvasRef, svgString]);

  const getLottieJson = useCallback(() => {
    if (!canvasSize.width || !canvasSize.height || dotsRef.current.length === 0)
      return null;
  ...
  }, [animationSettings, settings, canvasSize]);

  return { getSvgString, getLottieJson, svgString, canvasSize };
};
